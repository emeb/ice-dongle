...
#define spi_cs_low(s,bit) ((s)->SPICSR=~(0x01<<bit))
#define spi_cs_high(s) ((s)->SPICSR=0xff)

/* spi functions */
void spi_init(SPI_TypeDef *s);
void spi_tx_byte(SPI_TypeDef *s, uint8_t data, uint8_t csbit);
uint8_t spi_txrx_byte(SPI_TypeDef *s, uint8_t data, uint8_t csbit);
