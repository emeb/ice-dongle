...
# This target file provides a minimal LiteX SoC for the ice-dongle with a CPU,
# its ROM (in SPI Flash), its SRAM, close to the others LiteX targets.
# For more complete example of LiteX SoC for the ice-dongle with more features and
...
class BaseSoC(SoCCore):
    def __init__(self, bios_flash_offset, sys_clk_freq=24e6, revision="v1", **kwargs):
        platform = ice_dongle_platform.Platform(revision=revision)
        ...
        self.add_spi_flash(mode="4x", module=W25Q64JV(Codes.READ_1_1_4), with_master=False)
...
def main():
    ...
    parser = LiteXArgumentParser(platform=ice_dongle_platform.Platform, description="LiteX SoC on iCEBreaker.")
    ...
    parser.add_target_argument("--revision",          default="v0",             help="Board revision (v0 or v1).")
